...
var config = {
	...
	dbName : "bGeoCache",
	iOSService : null,
	timerServiceUrl : "backgroundGeo/Ti.Geo.Timer.js",
...
var helpers = {
	...
	recordLocation : function(e){
	  // JMHvar isValid = (((e.latitude!=undefined) && (e.latitude!=null)) && ((e.longitude!=undefined) && (e.longitude!=null)));
    var isValid = (((e.coords.latitude!=undefined) && (e.coords.latitude!=null)) && ((e.coords.longitude!=undefined) && (e.coords.longitude!=null)));
		if(!isValid){
			helpers.recordError({message:'Invalid Lat/Lng provided'});
			return;
		}
		var sql = 'INSERT INTO RECORD_CACHE_TBL (LATITUDE, LONGITUDE, ALTITUDE, RECORD_DATE) VALUES(?,?,?,?)';		
		var db = Ti.Database.open(config.dbName);
		db.execute(sql,e.coords.latitude,e.coords.longitude,e.coords.altitude,(new Date().getTime()));
		db.close();
		db = null;	
	...
	locationCallback : function(e){
		...
		// JMH helpers.recordLocation({latitude:e.coords.latitude,longitude:e.coords.longitude});		
    helpers.recordLocation({coords:e.coords});   
...
exports.setup = function(){
	...
	db.execute('CREATE TABLE IF NOT EXISTS RECORD_CACHE_TBL (RECORD_ID INTEGER PRIMARY KEY  AUTOINCREMENT  NOT NULL, LATITUDE DOUBLE, LONGITUDE DOUBLE, ALTITUDE DOUBLE, RECORD_DATE DOUBLE)');
...
exports.readCache = function(){
	var rows = [];
	helpers.setupCheck();
	var selectSQL = 'SELECT RECORD_ID, LATITUDE, LONGITUDE, ALTITUDE, RECORD_DATE FROM RECORD_CACHE_TBL ORDER BY RECORD_DATE';
	var db = Ti.Database.open(config.dbName);
	...
	if(getRows.getRowCount()>0){
		while (getRows.isValidRow()){
			rows.push({
				...
				altitude : parseFloat(getRows.field(3)),
				recordDate : new Date(parseFloat(getRows.field(4)))
			});
		}
	}				
