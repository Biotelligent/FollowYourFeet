...
var serviceObject = {
    ...
    counterId : null,
    lookupFrequency : Ti.App.Properties.getDouble('__bGeo_frequency', 3),
    lastLookupDate : null
};
...
if(_isAndroid){
	...
	Ti.App.currentService.addEventListener('stop',function(){
		Ti.API.info("Ti.Geo.Timer: background service is stopped");
	});

	serviceObject.counterId = setInterval(function()
	{
	    serviceObject.counter++;     
	    Ti.API.debug('Ti.Geo.Timer: Service Timer Count ' + serviceObject.counter);
